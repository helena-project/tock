...
impl<F: hil::flash::LegacyFlash> hil::flash::LegacyClient<F> for MuxFlash<'_, F> {
    fn read_complete(&self, pagebuffer: &'static mut F::Page, error: hil::flash::LegacyError) {
    ...
    fn write_complete(&self, pagebuffer: &'static mut F::Page, error: hil::flash::LegacyError) {
        self.inflight.take().map(move |user| {
    ...
    fn erase_complete(&self, error: hil::flash::LegacyError) {
        self.inflight.take().map(move |user| {
